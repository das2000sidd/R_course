...
# task 2
multi$edad1sex.cat <- cut(multi$edad1sex, c(-Inf, 15, 18, 19, 25, Inf),
                          labels = c("<15", "15-18", "18-19", "19-25", ">25"))
...
prop.table(table(multi$pais, multi$status), 1)

library(Epi)
stat.table(list(pais, status), list(N = count(), 
                `%` = percent(status)), data = multi, 
           margins = T)

# task 6
prop.table(table(multi$vph, multi$status), 1)
